...
Limitations:
Requires the input file to be formated properly."~" used for parsing, will crash reloading the file.

Test Plan:

File I/o Input: Check that it can accept no, one or 2 files. Check that it can read files and add them to the program. Test that it can write.

Input: Test that variations work (i.e "Q","q","Quit" etc.).  Test that inputting letters in number fields do not crash it. 
