...
$(document).ready(function() {
    ...
    $.ajax({
		...
		let invalidNames = data.stringInvalid.split(",");
		let i = null;
		
		if(arrayFileNames[0] == "")
		{

			uploadString = '<li>No valid files on the server.</li>'
		...
		for(i in invalidNames)
		{
			...
			if(invalidNames[i].length > 3)
			{

				uploadString = '<li>Invalid file '+  invalidNames[i] +'</li>';
				$('#StatusList').append(uploadString);
