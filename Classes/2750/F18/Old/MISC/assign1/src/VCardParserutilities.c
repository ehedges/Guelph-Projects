...
VCardErrorCode readToList(char* fileName,List* list)
{
	...
	while(fgets(inputString,750,ptr) != NULL)
	{
		...
		/*Checks to see if the string is longer than 75 if so it's an error*/
		if(strlen(inputString) > 77)
		{
...
VCardErrorCode createProperty(char*name, char*group, char*parameters, char*values, Property** prop)
{
	...
	if(parameters != NULL)
	{
		...
		for (int i = 0; i < strlen(parameters)+1; i++)
		{
			...
			if(parameters[i] == ';')
			{
				...
				else if(strcmp(tempParameterValue,"") == 0)
				{
				...
				if(strlen(tempParameterValue) == 0 || strlen(tempParameterName) == 0)
				{
	...
	for (int i = 0; i < strlen(values)+1; i++)
	{
		...
		else if((values[i] == ',' || values[i] == ';') && backlashBool == 0)
		{
