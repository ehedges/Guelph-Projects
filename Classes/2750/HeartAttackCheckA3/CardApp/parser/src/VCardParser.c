...
char* nameNumJSON()
{
	...
	char* string;
	char* fileName;
	char* stringFileNames;
	char* stringFileLength;
	char* stringName;
	char* stringInvalid;
	...
	stringFileNames = malloc(sizeof(char)*100);
	stringFileLength = malloc(sizeof(char)*100);
	stringName = malloc(sizeof(char)*100);
	stringInvalid = malloc(sizeof(char)*100);

	strcpy(stringFileNames,"\"fileNames\":\"");
	strcpy(stringFileLength,",\"fileLengths\":\"");
	strcpy(stringName,",\"stringNames\":\"");
	strcpy(stringInvalid,",\"stringInvalid\":\"");
	...
	if (d)
	{
		...
		while ((dir = readdir(d)) != NULL)
		{
			...
			fileName = mergeStrings(fileName,dir->d_name);
			error = createCard(fileName,(&obj));			
			
			if(error == OK)
			{

				if(strcmp(stringFileNames,"\"fileNames\":\"") != 0)
				{

					strcat(stringFileNames,",");
					strcat(stringFileLength,",");
					strcat(stringName,",");

				}

				lengthProperties = getLength(obj->optionalProperties);	
				...
				stringFileNames = mergeStrings(stringFileNames,dir->d_name);				
				sprintf(stringNum,"%d",lengthProperties);
				stringFileLength = mergeStrings(stringFileLength,stringNum);

				iter = createIterator(obj->fn->values);
				element = nextElement(&iter);

				stringName = mergeStrings(stringName,(char*)element);
				deleteCard(obj);
				free(obj);
			...
			else
			{
				...
				if(extensionCheck(fileName,"vcf",4) == 0 || extensionCheck(fileName,"vcard",6) == 0) 
				{

					if(strcmp(stringFileNames,",\"stringInvalid\":\"") != 0)
					{

						strcat(stringInvalid,",");
					
					}

					stringInvalid = mergeStrings(stringInvalid,dir->d_name);
					remove(fileName);
	...
	string = mergeStrings(string,stringFileNames);
	string = mergeStrings(string,stringFileLength);
	string = mergeStrings(string,stringName);
	string = mergeStrings(string,stringInvalid);

	free(stringInvalid);
	free(stringFileNames);
	free(stringFileLength);
	free(stringName);
...
char* VCardtoJSON(Card* card)
{
	...
	string = propToJSON(card->fn);

	iter = createIterator(card->optionalProperties);
	...
	if(card->birthday != NULL)
	{
		...
		string = mergeStrings(string,"~B~");

		tempString = dtToJSON(card->birthday);
	...
	if(card->anniversary != NULL)
	{
		...
		string = mergeStrings(string,"~A~");

		tempString = dtToJSON(card->anniversary);
