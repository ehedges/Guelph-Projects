...
char* nameNumJSON()
{
	...
	if (d)
	{
		...
		while ((dir = readdir(d)) != NULL)
		{
			...
			printf("HERER");
			if((extensionCheck(fileName,"vcf",4) == 0 && error == OK)) 
			{
				printf("HERER2");
				

				if(strcmp(invalidNames,",\"invalidNames\":\"") != 0)
				{
				...
				invalidNames = mergeStrings(fileName,dir->d_name);

			}

			if((extensionCheck(fileName,"vcard",6) == 0 && error == OK)) 
			{
				printf("HERER2");
	...
	string = mergeStrings(string,invalidNames);
